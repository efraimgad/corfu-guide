// Basic offline support: cache-first for the app shell (this file's own
// assets), stale-while-revalidate for everything else same-origin, and a
// deliberate pass-through (no caching at all) for Supabase API calls and
// third-party images — the app already has its own offline queue for
// Supabase writes (see js/sync.js), and pexels/unsplash/loremflickr photos
// are too numerous to be worth the storage, with the broken-image SVG
// fallback in js/init.js already covering a failed fetch either way.

const CACHE_NAME = 'corfu-guide-v1';
...
self.addEventListener('fetch', (event) => {
    ...
    if (sameOrigin && APP_SHELL.some((path) => request.url.endsWith(path))) {
        // App shell: cache-first, so the page loads instantly offline.
        event.respondWith(
