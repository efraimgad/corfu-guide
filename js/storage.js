...
async function initCloudSync() {
    // Not configured -> nothing to sync with. Return before touching the
    // network so there is no failed request and no misleading warning.
...
// CORFU_LOCATIONS (see the "Deferred" itinerary/FAQ/activities/trip-planning/
// health-safety/language-daily extraction), while still hydrating every
// card's widget with saved state.
...
function renderPersonalTrackingWidget(itemId) {
    ...
    widget.querySelectorAll('.pt-star').forEach(star => {
        ...
        // Visual fill is cumulative (every star up to the rating lights up),
        // but role="radio" is a single-choice control - only the star
        // matching the rating exactly is the "checked" one.
