...
// Ids generated by generateReservationId() below always match this. Anything
// else came from somewhere that is not this app.
const RESERVATION_ID_RE = /^res-\d+-[a-z0-9]+$/;
...
function getReservations() {
    try {
        ...
        // Only the array-ness used to be checked, never the entries. Each
        // record's `id` is interpolated into an onclick handler by
        // reservationRowHtml() below, so a crafted id from a tampered
        // localStorage executed as JavaScript the moment the traveller tapped
        // edit or delete on their own saved reservation. escapeAttr() now
        // escapes the quote that made that possible (js/html-utils.js); this
        // is the second half — a malformed id is replaced rather than
        // round-tripped back into rendered markup, so the record stays usable
        // instead of being silently dropped.
        return parsed
            .filter(r => r && typeof r === 'object')
            .map(r => (typeof r.id === 'string' && RESERVATION_ID_RE.test(r.id)
                ? r
                : { ...r, id: generateReservationId() }));
    } catch (e) {
        return [];
    }
}
