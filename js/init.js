...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // An "#item=..." deep link is the most specific intent there is, so it
    // wins over both the hash-tab and the first-visit default. It needs the
    // category rendered before it can scroll to the card, which
    // switchTab() -> ensureTabRendered() inside openItemFromHash() handles.
    const itemId = getItemIdFromHash();
    const openedItem = itemId ? openItemFromHash() : false;
    ...
    // actually arrived with should stick - which includes an "#item=..."
    // deep link, so clearing is skipped there too or the shared URL would
    // be erased from the address bar the instant it opened.
    if (!hashTab && !openedItem) {
    ...
    // Fill in any category the visitor hasn't opened, once the browser is
    // idle - keeps every card discoverable to crawlers/Ctrl+F/screen
    // readers without putting them back on the critical path.
    scheduleRemainingCategoryRender();
});
