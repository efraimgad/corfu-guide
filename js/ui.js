...
// Phase C2: the four legacy category tabs (beaches/food/attractions/gems)
// were deleted; the unified Explore tab replaces them. Rather than editing
// the ~15 switchTab('beaches') call sites scattered through dashboard
// quick-nav buttons, hash routes and #cat links, they are redirected here -
// exactly the pattern GT_GUIDE_PANEL_IDS already uses for the merged Guide
// panels. Editing call sites risks missing one silently; a redirect map
// cannot miss any, and keeps #beaches etc. working as bookmarks/hash routes.
const GT_LEGACY_CATEGORY_TABS = {
    beaches: 'beaches',
    food: 'food',
...
function switchTab(tabId, skipScroll) {
    ...
    // Same redirect shape as the Guide panels above: a legacy category id
    // becomes "Explore, with that category selected".
    let gtExploreCategoryToShow = null;
    if (GT_LEGACY_CATEGORY_TABS[tabId]) {
        gtExploreCategoryToShow = GT_LEGACY_CATEGORY_TABS[tabId];
        tabId = 'explore';
    }
    ...
    if(targetTab) {
        ...
        setTimeout(() => {
            ...
            // Phase C2: setupScrollSpy() went with <section id="food">; it
            // tracked that section's per-category headings, which no longer
            // exist. Explore does its own sticky-header handling.
            ...
            // the (CSS-hidden at that width) toggle button - see
            // ensureExploreMapVisible()'s own comment in js/map.js.
            if (tabId === 'explore' && typeof window.matchMedia === 'function'
            ...
            // A redirected legacy category (#beaches -> Explore/beaches).
            // Applied here, after #explore is visible, for the same reason
            // the Guide panel is: selecting before the tab is displayed
            // would measure sticky offsets against a hidden container.
            if (tabId === 'explore' && gtExploreCategoryToShow
                && typeof selectExploreCategory === 'function') {
                selectExploreCategory(gtExploreCategoryToShow, null);
            }
        }, 10);
    }
    ...
    // Phase D: the FAB follows the canonical map, which is now Explore's.
    // Home has its own full-bleed map that is always visible and needs no
    // toggle, so the FAB stays hidden there rather than offering a control
    // for something already on screen.
    const mapFabBtn = document.getElementById('map-fab-btn');
    if (mapFabBtn) mapFabBtn.classList.toggle('map-fab-visible', tabId === 'explore');
}
