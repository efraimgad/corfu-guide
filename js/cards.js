...
function buildVerifiedInfoHTML(d) {
    ...
    if (d.phone) {
        ...
        rows.push(`<p class="verified-row">📞 <a href="tel:${escapeAttr(dial)}" class="verified-tel" dir="ltr">${escapeHtml(formatPhone(dial))}</a></p>`);
    } else if (d.phoneNote) {
        rows.push(`<p class="verified-row verified-muted">📞 ${escapeHtml(d.phoneNote)}</p>`);
    }
    ...
    if (d.priceFlag) {
        rows.push(`<p class="verified-row verified-warn">⚠️ ${escapeHtml(d.priceFlag)}</p>`);
    }
    if (d.verifyNote) {
        rows.push(`<p class="verified-row verified-warn">⚠️ ${escapeHtml(d.verifyNote)}</p>`);
    }
    ...
    const stamp = d.verifiedOn
        ...
        : `<p class="verified-stamp verified-warn">⚠️ שעות לא מאומתות — כדאי לבדוק לפני ההגעה</p>`;
...
function renderAttractionCard(d, index) {
    const numberedTitle = typeof index === 'number' ? `${index + 1}. ${d.title}` : d.title;
    return `<article class="${d.cardClass}" data-id="${escapeAttr(d.id)}" data-tags="${escapeAttr(d.tags)}"${metaAttrs(d)}>
      ...
      <div class="p-4 flex-grow space-y-3">
        <h3 class="text-lg font-bold text-gray-900">${escapeAttr(numberedTitle)}</h3>${d.bodyHtml}
        ${buildVerifiedInfoHTML(d)}
      </div>
      ${buildPersonalTrackingWidgetHTML()}
    </article>`;
...
function renderAllAttractions() {
    ...
    if (!grid || !window.CORFU_LOCATIONS) return;
    grid.innerHTML = window.CORFU_LOCATIONS.attractions.map((d, i) => renderAttractionCard(d, i)).join('\n');
}
