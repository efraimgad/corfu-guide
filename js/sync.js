...
// rather than sitting on a permanent "pending sync" that will never clear.
// Local persistence in storage.js is unaffected — it never went through here.
const CLOUD_ON = !!window.SUPABASE_ENABLED;
...
function enqueue(type, key, entry) {
    ...
    saveSyncQueue(queue);
    if (!navigator.onLine) showOfflineToastOnce();
    updateSyncIndicator();
}
...
// --- Sync status indicator (bottom-left dot) + offline toast --------------
// Reflects three states: offline (red), pending (gold - queue non-empty),
// online (olive - queue empty). "syncing" (spinning) is shown only for the
// brief window while flushSyncQueue is actively running.
function updateSyncIndicator(state) {
    ...
    if (!state) {
        state = !navigator.onLine ? 'offline' : (getSyncQueue().length > 0 ? 'pending' : 'online');
    }
    const labels = {
    ...
    el.setAttribute('title', labels[state] || labels.online);
}

let offlineToastShownThisSession = false;
window.addEventListener('online', () => { offlineToastShownThisSession = false; });

function showOfflineToastOnce() {
    if (!CLOUD_ON) return;
    if (offlineToastShownThisSession) return;
    offlineToastShownThisSession = true;
    const toast = document.getElementById('offline-toast');
    if (!toast) return;
    toast.textContent = 'מצב לא מקוון — השינויים יישמרו כשתתחברו מחדש';
    toast.classList.add('offline-toast-visible');
    setTimeout(() => toast.classList.remove('offline-toast-visible'), 4000);
}
...
async function queueItemStateSync(itemId) {
    ...
    try {
        ...
        if (window.CorfuStorage) window.CorfuStorage.clearItemDirty(itemId);
    } catch (e) {
        enqueue('item_state', itemId, { itemId });
    }
}
...
async function queueItineraryDaySync(dayNumber, completed) {
    ...
    try {
        ...
        if (window.CorfuStorage) window.CorfuStorage.clearDayDirty(dayNumber);
    } catch (e) {
        enqueue('itinerary_day', dayNumber, { dayNumber, completed });
    }
}
...
async function flushSyncQueue() {
    ...
    if (queue.length > 0) updateSyncIndicator('syncing');
    for (const entry of queue) {
        try {
            if (entry.type === 'item_state') {
                await window.CorfuDB.upsertItemState(entry.itemId, buildItemStatePayload(entry.itemId));
                if (window.CorfuStorage) window.CorfuStorage.clearItemDirty(entry.itemId);
            } else if (entry.type === 'itinerary_day') {
                await window.CorfuDB.upsertItineraryDay(entry.dayNumber, entry.completed);
                if (window.CorfuStorage) window.CorfuStorage.clearDayDirty(entry.dayNumber);
            }
            dequeue(entry.type, entry.key); // dequeue() also refreshes the indicator
        } catch (e) {
            break;
        }
    }
...
if (CLOUD_ON) setTimeout(flushSyncQueue, 1500);

window.CorfuSync = { queueItemStateSync, queueItineraryDaySync, flushSyncQueue, getSyncQueue, updateSyncIndicator };
