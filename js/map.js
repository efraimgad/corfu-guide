...
// Opening the map pulls ~150KB of Leaflet + marker-cluster from a CDN on
// first use, which on a slow connection left the panel visibly empty with
// no indication anything was happening. These two states cover the wait
// and the failure; role="status" so the change is announced rather than
// only shown.
function showMapLoadingState() {
    const el = document.getElementById('beach-map');
    if (!el) return;
    el.innerHTML = '<div class="map-state" role="status"><span class="map-state-spinner" aria-hidden="true"></span><p>טוען את המפה…</p></div>';
}

function showMapErrorState() {
    const el = document.getElementById('beach-map');
    if (!el) return;
    el.innerHTML = '<div class="map-state" role="status"><p>לא ניתן לטעון את המפה כרגע (חיבור אינטרנט נדרש).</p></div>';
}
...
function initBeachMap() {
    ...
    // Clear the loading placeholder before Leaflet takes over the
    // container, so its panes aren't layered on top of leftover nodes.
