...
// Hebrew weekday abbreviation ("ה׳") for the day headers, computed from the
// real calendar (via Intl, in the trip's own Athens timezone) rather than
// hand-typed per day - see WEEKDAY_CODE_FROM_SHORT in js/itinerary.js for
// the matching SU/MO/.../SA codes used by closedDays cross-checking.
const HE_WEEKDAY_ABBR = { Sun: 'א׳', Mon: 'ב׳', Tue: 'ג׳', Wed: 'ד׳', Thu: 'ה׳', Fri: 'ו׳', Sat: 'ש׳' };
function formatWeekdayHe(date) {
    const enShort = new Intl.DateTimeFormat('en-US', { timeZone: TRIP_TIMEZONE, weekday: 'short' }).format(date);
    return HE_WEEKDAY_ABBR[enShort] || '';
}

// When the page content was last substantively updated - shown in the
// hero's "עודכן לאחרונה" badge. A hardcoded date that just sits in the
// HTML can silently go stale; this is the one place to change it.
const LAST_UPDATED = new Date('2026-07-01T00:00:00+03:00');
...
function injectTripDates() {
    ...
    set('hero-last-updated', formatMonthYearHe(LAST_UPDATED));
    set('currency-rates-updated', formatMonthYearHe(LAST_UPDATED));
    ...
    // everywhere else, plus the real Hebrew weekday abbreviation, so it can
    // never drift from - or contradict - the dashboard's own countdown.
    for (let day = 2; day <= 6; day++) {
        const date = dateForTripDay(day);
...
// the HTML (so the page still reads correctly with JS disabled) - this
// overwrites them with the real, current counts on load, so they can
// never silently drift from window.CORFU_LOCATIONS as items are added.
function injectCounts() {
    ...
    set('hero-count-beaches', (locations.beaches || []).length);
    set('hero-count-food', (locations.food || []).length);
    set('hero-count-attractions', (locations.attractions || []).length);
    set('hero-count-gems', (locations.gems || []).length);

    set('nav-count-beaches', (locations.beaches || []).length);
    set('nav-count-food', (locations.food || []).length);
    set('nav-count-attractions', (locations.attractions || []).length);
...
// fallbacks shown when that file is absent. Either default is shown
// until the traveler edits it via the dashboard editor, at which point
// their edit (in localStorage) takes priority.
const DEFAULT_HOTEL = (window.TRIP_PRIVATE && window.TRIP_PRIVATE.hotel) ||
    { name: 'המלון שלכם', note: 'הוסיפו פרטים ב-js/trip-private.js' };
window.DEFAULT_HOTEL = DEFAULT_HOTEL; // read by js/map.js for the hotel marker popup
...
// leaving the generic Hebrew fallback text already sitting in the markup
// untouched when it isn't (this committed build never ships real booking
// data - see js/trip-private.js in .gitignore). Reuses DEFAULT_HOTEL,
// which already encodes exactly that same TRIP_PRIVATE-or-fallback logic
// for the dashboard's hotel/car editor tiles, so both halves of the page
// can never disagree about whether a hotel/car has actually been booked.
function fillTripPrivateHooks() {
    const hotelName = (DEFAULT_HOTEL && DEFAULT_HOTEL.name) || 'המלון שלכם';
    document.querySelectorAll('.trip-private-hotel-name').forEach(el => {
        el.textContent = hotelName;
    });

    const car = window.TRIP_PRIVATE && window.TRIP_PRIVATE.car;
    let carRef = 'פרטי ההזמנה שלכם';
    if (car && (car.name || car.note)) {
        carRef = [car.note, car.name].filter(Boolean).join(', ');
    }
    document.querySelectorAll('.trip-private-car-ref').forEach(el => {
        el.textContent = carRef;
    });
}
...
// `current_weather=true` param only ever returns *today's* current
// conditions, not a multi-day forecast - Open-Meteo does offer a `daily`
// param with real per-day highs/lows/weathercode, but wiring up an actual
// 7-day forecast UI is a bigger change than this fix (new response
// shape, per-day rendering, more failure/partial-data states to handle
// honestly) and is intentionally left as a follow-up rather than guessed
// at here. The monthly-averages table in #plan-weather already covers
// "what's the weather like" for trip-planning purposes; this widget is
// scoped to "what does it look like right now".
//
// If the fetch fails for any reason (offline, no reception, the API
// itself down or slow, a malformed response) OR simply never resolves
// (a hung connection - fetch() has no built-in timeout), show a static,
// honest "couldn't load" state with a link straight to that monthly
// table - never leave the widget stuck on "טוען..." forever.
const DASH_WEATHER_TIMEOUT_MS = 8000;
...
function fetchDashWeather() {
    ...
    const fallbackLinkEl = document.getElementById('dash-weather-fallback-link');
    if (!valEl) return;

    let settled = false;
    const showFailure = () => {
        if (settled) return; // the real fetch already won the race - don't overwrite a success
        settled = true;
    ...
    fetch('https://api.open-meteo.com/v1/forecast?latitude=39.6243&longitude=19.9217&current_weather=true')
        ...
        .then(data => {
            ...
            if (settled) return; // timeout already fired and rendered the failure state
            settled = true;
...
// Hotel / car rental editor — saved locally only. Both default to this
// trip's confirmed bookings (DEFAULT_HOTEL / DEFAULT_CAR) until the
// traveler edits them, at which point the edit is saved to localStorage
// and takes priority over the default.
...
function openDashEditor(type) {
    ...
    const entry = saved[type] || (isHotel ? DEFAULT_HOTEL : DEFAULT_CAR);
...
function loadDashEditorState() {
    ...
    renderDashEntry('car', saved.car || DEFAULT_CAR);
}
