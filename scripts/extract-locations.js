...
// REFUSE TO WRITE A SMALLER DATASET THAN THE ONE ALREADY ON DISK.
//
// This script scrapes #beaches-grid / #attractions-grid / #gems-container-grid
// out of index.html. Phase C2 deleted those sections when the four category
// tabs were folded into Explore, so the selectors now match nothing and a run
// would overwrite 169 records with an empty structure — silently, because
// writeFileSync neither knows nor cares. scripts/recolor-badges.js:9-10 already
// carries a hand-written warning about exactly this ("confirmed - do not run it
// again"); a comment is not a safeguard.
//
// js/locations-data.js is now hand-maintained (the audit removed three
// generator-only fields from it and rewrote 87 mapsUrl values), so an
// accidental regeneration is a data-loss event, not an inconvenience.
const counts = Object.keys(results).reduce((a, k) => (a[k] = (results[k] || []).length, a), {});
const total = Object.values(counts).reduce((a, b) => a + b, 0);
const targetPath = path.join(ROOT, 'js', 'locations-data.js');

// Counted by EVALUATING the file, not by JSON.parse: it is a .js file and does
// legitimately carry // comments, so it is not strict JSON and never was.
let existingTotal = 0;
if (fs.existsSync(targetPath)) {
    try {
        ...
        vm.runInContext(fs.readFileSync(targetPath, 'utf8'), ctx, { timeout: 5000 });
        const prev = ctx.window.CORFU_LOCATIONS || {};
        existingTotal = Object.values(prev).reduce((a, v) => a + (Array.isArray(v) ? v.length : 0), 0);
    } catch (e) {
        console.warn('Could not read the existing dataset to compare against: ' + e.message);
    }
}

if (total === 0 || (existingTotal > 0 && total < existingTotal)) {
    console.error('\nREFUSING TO WRITE js/locations-data.js.');
    console.error('  extracted: ' + total + ' records ' + JSON.stringify(counts));
    console.error('  on disk:   ' + existingTotal + ' records');
    console.error('\nThe source sections this script scrapes (#beaches-grid, #attractions-grid,');
    console.error('#gems-container-grid) were removed from index.html in Phase C2, so this script');
    console.error('can no longer regenerate the dataset. Fix its selectors before using it, or');
    console.error('edit js/locations-data.js directly. Nothing was written.');
    process.exit(1);
}

const out = banner + 'window.CORFU_LOCATIONS = ' + JSON.stringify(results, null, 2) + ';\n';
fs.writeFileSync(targetPath, out, 'utf8');
console.log('Wrote js/locations-data.js (' + total + ' records)');
