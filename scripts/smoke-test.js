...
async function main() {
    ...
    const renderedCounts = {
        beaches: document.querySelectorAll('#beaches-grid > *').length,
        attractions: document.querySelectorAll('#attractions-grid > *').length,
        gems: document.querySelectorAll('#gems-container-grid > *').length,
        // Food cards are split across several per-category grids inside
        // #food (no single food-grid container), so count every rendered
        // card by its data-id attribute instead.
        food: document.querySelectorAll('#food [data-id]').length
    };
    Object.keys(renderedCounts).forEach((category) => {
        const expected = (locations[category] || []).length;
        const actual = renderedCounts[category];
        if (actual === expected) ok(`#${category}: rendered card count (${actual}) matches CORFU_LOCATIONS.${category}.length`);
        else fail(`#${category}: rendered ${actual} card(s), but CORFU_LOCATIONS.${category}.length is ${expected}`);
    });
    ...
    const dinnerHooks = Array.from(document.querySelectorAll('[data-dinner-food-id]'));
    if (dinnerHooks.length === 0) {
        fail('no [data-dinner-food-id] dinner hooks found in the rendered DOM');
    } else {
        const foodIds = new Set((locations.food || []).map((d) => d.id));
        const badDinnerHooks = dinnerHooks.filter((el) => !foodIds.has(el.getAttribute('data-dinner-food-id')));
        if (badDinnerHooks.length === 0) {
            ok(`all ${dinnerHooks.length} [data-dinner-food-id] hooks resolve to a real CORFU_LOCATIONS.food record`);
        } else {
            badDinnerHooks.forEach((el) => fail(`[data-dinner-food-id="${el.getAttribute('data-dinner-food-id')}"] does not match any CORFU_LOCATIONS.food id`));
        }
    }

    const priceFlagHooks = Array.from(document.querySelectorAll('[data-price-flag-id]'));
    if (priceFlagHooks.length === 0) {
        fail('no [data-price-flag-id] hooks found in the rendered DOM');
    } else {
        const attractionIds = new Set((locations.attractions || []).map((d) => d.id));
        const badPriceHooks = priceFlagHooks.filter((el) => !attractionIds.has(el.getAttribute('data-price-flag-id')));
        if (badPriceHooks.length === 0) {
            ok(`all ${priceFlagHooks.length} [data-price-flag-id] hooks resolve to a real CORFU_LOCATIONS.attractions record`);
        } else {
            badPriceHooks.forEach((el) => fail(`[data-price-flag-id="${el.getAttribute('data-price-flag-id')}"] does not match any CORFU_LOCATIONS.attractions id`));
        }
    }
